(function () {
    ...
    function initRibbonBar() {
	    ...
	    var  ExcelQueryParameter;//mark  new
        ...
        $(".ribbon-bar").gcuiribbon({
            click: function (evt, cmd) {
                ...
                var selections = sheet.getSelections(), singleCellSelection;
                ...
                switch (cmd.name) {
				  ...
				  switch (cmd.commandName) {
                    case "Excel-Query-Parameter":
                        if(getQueryVariable("target")){
                            if (ExcelQueryParameter === undefined) {
                                ExcelQueryParameter = new designer.ExcelQueryParameter();
                            }
                            ExcelQueryParameter.open();
                        }else{
                            designer.MessageBox.show('未发现参数信息', '查询提示', 1 /* info */, 1 /* okCancel */, function (e, dialogResult) {
                                switch (dialogResult) {
                                    case 1 /* ok */:
                                        break;
                                    case 4 /* cancel */:
                ...
                switch (cmd.commandName) {
                    ...
                    case "name-manager":
                        ...
                        break;
                    case "show-formulas":
                        designer.actions.doAction("setShowFormulas", spread, $("#show-formulas").prop("checked"));
                        break;
                    ...
                    case "insert-hyperlink":
                        ...
                        if (designer.dialog.hyperlinkDialog === undefined) {
                            designer.dialog.hyperlinkDialog = new designer.HyperlinkDialog();
                        }
                        designer.dialog.hyperlinkDialog.open();
                        break;
                    case "insert-shapes":
                    ...
                    case "torange-table":
                        ...
                        break;
                    case "table-allow-auto-expand":
                        if (!activeTable) {
                            break;
                        }
                        toggleCheckBoxIcon("table-allow-auto-expand");
                        designer.actions.doAction("setTableAllowAutoExpand", spread, getCheckBoxIconStatus("table-allow-auto-expand"));
                        break;
    ...
    function updateFormat() {
        ...
        if (formatter && formatter.formatString){
            formatter = formatter.formatString(); // Fix SJS-3588, add the support for GeneralFormatter
        }
        if (!formatter || formatter.toLowerCase() === 'general') {
            setDropDownContent('#number-format', designer.res.generalFormat);
        } else if (_isNumberFormat(formatter)) { //To fix SJS-3148, display the Number format correctly.
            setDropDownContent('#number-format', designer.res.commonFormats.Number.label);
        } else if (_isAccountingFormat(formatter)) { //To fix SJS-3607, display the Accounting format correctly.
    ...
    function updateTable() {
        ...
        if (sheet.tables.find(sheet.getActiveRowIndex(), sheet.getActiveColumnIndex()) instanceof Sheets.Tables.Table) {
            ...
            setCheckBoxIconStatus("table-allow-auto-expand", table.allowAutoExpand());
    ...
    function updateShowHide() {
        ...
        $('.ribbon-bar').gcuiribbon('setButtonChecked', 'show-formulas', sheet.options.showFormulas);
    }
    ...
    function attachEvent(rebind) {
        ...
        if (!ribbon.eventAttached) {
            $(designer).bind("updateRibbon", function (event, data) {
                updateCellStyle();
                $('.ribbon-bar').gcuiribbon('setButtonChecked', 'show-formulas', spread.getActiveSheet().options.showFormulas);
            });
