(function () {
    ...
    util.resolveHtmlPath = function (directory, filename) {
        if (typeof(baseRoot) != "undefined") {
            return baseRoot+directory.slice(3,directory.length)+"/"+filename
        }
            ...
            if (chart.chartType() === ChartType.sunburst || chart.chartType() === ChartType.treemap) {
                ...
                for (i = 0; i < seriesItemCount; i++) {
                    ...
                    if (!options.type) {
                        ...
                        if (seriesItem.trendlines) {
                            for (var j=0; j<seriesItem.trendlines.length; j++) {
                                var trendline = seriesItem.trendlines[j];
                                if (trendline.style) {
                                    trendline.style.color = color;
                                } else {
                                    trendline.style = {"color": color};
                                }
                            }
        ...
        getSeriesWithoutStyle: function (series) {
            ...
            for (; i < count; i++) {
                var seriesItemWithoutStyle = {
                    ...
                    plotVisibleOnly: series[i].plotVisibleOnly
                };
